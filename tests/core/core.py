...
d2 = g.grid([16,16], g.double)

c = g.complex(d2)
Z = np.ones((16,16))

p = 2.0 * np.pi * np.array([2,0]) / 16


for i in range(16):
    for j in range(16):
        # f = 1/(2.0*np.pi)*np.exp(-0.1*((i-4)**2+(j-8)**2))
        f = np.exp(-1.0j*(p[0]*i+p[1]*j))
        # print(" x = ({},{}), f(x) = {}".format(i,j,f))
        c[i,j]=f
        Z[i,j]=f
...
fft_c = g.eval(g.fft()* c)

print(fft_c)

# for i in range(16):
#     for j in range(16):
#         Z[i,j] = (fft_c[i,j]).real
...
# for i in range(16):
#     for j in range(16):
#         print("({},{}): fft_f = {}", i-8, j-8, fft_c[i,j])


# fft_l_dp = g.eval(g.fft() * l_dp)
# eps = g.norm2(g.adj(g.fft()) * fft_l_dp - l_dp) / g.norm2(l_dp)
# g.message("FFTinv * FFT:", eps)
# assert eps < 1e-12

# eps = g.norm2(g.sum(exp_ixp * l_dp) / np.prod(L) - fft_l_dp[1, 2, 3, 4])
# g.message("FFT forward test:", eps)
# assert eps < 1e-12

# # fft_mom_A = g.slice(
# #     g.exp_ixp(2.0 * np.pi * np.array([1, 2, 3, 0]) / L) * l_dp, 3
# # ) / np.prod(L[0:3])
# fft_mom_B = [g.vcolor(x) for x in g.eval(g.fft([0, 1, 2]) * l_dp)[1, 2, 3, 0 : L[3]]]


# print(g.eval(g.fft([0, 1, 2]) * l_dp))


# print(np.shape(fft_mom_B))
# for t in range(L[3]):
#     eps = g.norm2(fft_mom_B[t])
#     assert eps < 1e-12
